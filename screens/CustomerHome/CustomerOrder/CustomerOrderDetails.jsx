...
const CustomerOrderDetails = ({ navigation, route }) => {
    ...
    onSuccess: async (e) => {
      if (e.status === 200) {
        ...
        navigation.navigate("Chat", {
          ...
          loginUserId: orderDetails?.customerUser?._id,
          otherUser: {
            fullName: orderDetails?.vendorUser?.vendor?.fullName,
            profileImage: orderDetails?.vendorUser?.vendor?.profileImage,
          },
        });
      } else {
        showMessage({
  ...
  return (
    <>
        ...
        <View style={styles.proposalContainer}>
          ...
          >
            <View style={styles.proposalHeader}>
              <Image
                source={{
                  uri:
                    orderDetails?.vendorUser?.vendor?.profileImage ||
              ...
              <View>
                <Text style={styles.proposalTitle}>
                  {orderDetails?.vendorUser?.vendor?.fullName}
                </Text>
          ...
          <View style={styles.personalInfo}>
            ...
            <Text style={styles.infoText}>
              Phone: {orderDetails?.vendorUser?.vendor?.mobileNo}
            </Text>
            <Text style={styles.infoText}>
        ...
        <ScrollView>
          <View>
            <TouchableOpacity
              ...
              onPress={() =>
                Linking.openURL(
                  `tel:${orderDetails?.vendorUser?.vendor?.mobileNo}`
                )
              }
            >
            ...
            {orderDetails?.status === "complete" ? (
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      navigation.navigate("CreateReview", {
                        orderId: orderDetails?._id,
                        vendorId: orderDetails?.vendorUser?._id,
                      })
                    }
                  >
                    <MaterialCommunityIcons
              ...
              "cancel" ? null : orderDetails?.status === "disputed" ? (
              <TouchableOpacity
                style={styles.btn}
                onPress={() =>
            ...
            <View style={styles.reviewContainer}>
              {reviewByOrder ? (
                    ...
                    <TouchableOpacity
                      onPress={() =>
                        navigation.navigate("UpdateReview", {
                          reviewId: reviewByOrder?._id,
                          prevComment: reviewByOrder?.comment,
                          prevRating: reviewByOrder?.rating,
                        })
                      }
                    >
                      <MaterialCommunityIcons
                ...
                <Text style={styles.reviewTxt}>
                  Your feedback matters! Please share your experience with this
                  vendor to help others make informed choices.
                </Text>
              )}
            </View>
          ) : null}
      ...
      <CustomModal
        ...
        onAction={() => handleStatusBtn("complete")}
        action="Complete"
